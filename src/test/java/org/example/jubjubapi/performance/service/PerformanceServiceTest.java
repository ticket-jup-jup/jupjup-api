...
class PerformanceServiceTest {
    ...
    void 삭제된_회차_삭제() {
        ...
        when(programRepository.findAllByDeletedAtIsNull()).thenReturn(List.of(program));
        when(ticketServerClient.getPerformances(100L)).thenReturn(List.of(ticketServerPerformance));
        when(performanceRepository.findByExternalPerformanceId(1L))
        ...
        verify(performanceRepository).findAllByProgramIdAndDeletedAtIsNullAndExternalPerformanceIdNotIn(
                eq(program.getId()),
                eq(Set.of(1L))
        );
    }
    ...
    void 프로그램별_회차_동기화() {
        ...
        when(ticketServerClient.getPerformances(100L)).thenReturn(List.of());
        when(ticketServerClient.getPerformances(200L)).thenReturn(List.of());
        when(performanceRepository.findAllByProgramIdAndDeletedAtIsNullAndExternalPerformanceIdNotIn(
                eq(1L),
                eq(Set.of())
        )).thenReturn(List.of());
